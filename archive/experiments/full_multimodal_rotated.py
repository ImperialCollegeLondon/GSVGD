...
def mix_gauss_experiment(mixture_dist, means, R):
    ...
    cov[:2, :2] = torch.Tensor([[1, 0.8], [0.8, 1]])
    cov = R @ cov @ R.t()
    comp = D.MultivariateNormal(means.to(device), cov)
...
parser.add_argument('--nmix', type=int, default=5, help='no. of modes')
...
results_folder = f"./res/full_multimodal_seq_rotated/{args.kernel}_epoch{epochs}_lr{lr}_delta{delta}_n{nparticles}_dim{dim}"
if add_noise:
...
if __name__ == "__main__":
    ...
    mix_means = torch.zeros((1, dim), device=device)
    mix_means[:, :2] = 2
    R = torch.nn.init.orthogonal_(torch.empty(dim, dim, device=device))
    ...
    #! hard coded
    # lr_ls = [0.01, 0.01, 0.001]
    # delta_ls = [0.01, 1e-5, 0.001]
    def run_gsvgd(eff_dims):
        for i, eff_dim in enumerate(eff_dims):
            ...
            fig_gsvgd = plot_particles(
                ...
                savedir=results_folder + f"/fullgsvgd_effdim{eff_dim}_lr{lr}_delta{delta}_m{m}.png"
            )

            # store results
            # gsvgd.particles = [x @ R.cpu() for x in gsvgd.particles]
            res_gsvgd[i] = {
    ...
    # particles
    # svgd.particles = [x @ R.cpu() for x in svgd.particles]
    # maxsvgd.particles = [x @ R.cpu() for x in maxsvgd.particles]
