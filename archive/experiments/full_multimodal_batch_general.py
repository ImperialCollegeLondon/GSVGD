...
from src.gsvgd_general import FullGSVGDBatch
...
results_folder = f"./res/multimodal_batch_gen{args.suffix}/{args.kernel}_epoch{epochs}_lr{lr}_delta{delta}_n{nparticles}_dim{dim}"
...
if __name__ == "__main__":
    ...
    if args.method in ["GSVGD", "all"]:
        ...
        def run_gsvgd(eff_dims):
            for i, eff_dim in enumerate(eff_dims):
                ...
                A = torch.eye(dim, device=device).requires_grad_()
                # A = torch.nn.init.orthogonal_(
                #     torch.empty(x.shape[-1], x.shape[-1], device=device)
                # ).requires_grad_()
                A = A[:, :(m*eff_dim)]
                B = A.clone().requires_grad_()
                ...
                U, metric_gsvgd = gsvgd.fit(x_gsvgd, A, B, m, epochs, 
                    verbose=True, metric=metric_fn, save_every=save_every, threshold=0.0002*m)
    ...
    del res_gsvgd
    del maxsvgd
    sh = f"python plots/plot_metric_vs_epochs.py --exp=multimodal_batch_gen{args.suffix} --nparticles={nparticles} --epochs={epochs} --lr={lr} --delta={delta} --dim={dim}"
    os.system(sh)