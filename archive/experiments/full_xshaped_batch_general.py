...
correlation = 0.95
save_every = 10 # 200 # save particles
...
results_folder = f"./res/xshaped_batch_gen{args.suffix}/{args.kernel}_epoch{epochs}_lr{lr}_delta{delta}_n{nparticles}_dim{dim}"
        ...
        def run_gsvgd(eff_dims):
            for i, eff_dim in enumerate(eff_dims):
                ...
                U, metric_gsvgd = gsvgd.fit(x_gsvgd, A, B, m, epochs, 
                    verbose=True, metric=metric_fn, save_every=save_every, threshold=1e-3)
