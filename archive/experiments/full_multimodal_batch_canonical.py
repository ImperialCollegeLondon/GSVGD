...
dim = args.dim
lr = args.lr
lr_g = 0.1 # args.lr_g if args.lr_g is not None else lr #! hard coded
delta = args.delta
T = args.T
...
results_folder = f"./res/multimodal_batch_cano{args.suffix}/{args.kernel}_epoch{epochs}_lr{lr}_delta{delta}_n{nparticles}_dim{dim}"
    ...
    if args.method in ["GSVGD", "all"]:
        ...
        def run_gsvgd(eff_dims):
            for i, eff_dim in enumerate(eff_dims):
                ...
                # optimizer = optim.Adagrad([x_gsvgd], lr=lr)
                manifold = Grassmann(dim, eff_dim)
                U = torch.eye(dim, device=device).requires_grad_()
                U = U[:, :(m*eff_dim)]
                B = torch.eye(dim, device=device)
                B = B[:, :(m*eff_dim)]

                # B = torch.eye(dim, device="cuda")
                # score = torch.nn.init.orthogonal_(
                #     torch.empty(7, 10, device="cuda")
                # )
                # torch.allclose(score @ B, score)
                # torch.allclose((score.cpu() @ B.cpu()), score.cpu())
                ...
                U, metric_gsvgd = gsvgd.fit(X=x_gsvgd, U=U, B=B, m=m, epochs=epochs, 
